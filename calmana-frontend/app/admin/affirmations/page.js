...
export default function AdminAffirmationsPage() {
  ...
  useEffect(() => {
    ...
    axios.get("http://localhost:5000/api/admin-data/affirmations", { headers: { Authorization: `Bearer ${token}` } })
  ...
  const filtered = useMemo(() => {
    ...
    if (!t) return items;
    return items.filter(a => (a.userId?.username || "").toLowerCase().includes(t) || (a.text || "").toLowerCase().includes(t));
  }, [items, q]);
  ...
  function exportCSV() {
    ...
    const b = new Blob([csv], { type: "text/csv" }); const url = URL.createObjectURL(b); const a = document.createElement("a"); a.href = url; a.download = "affirmations.csv"; a.click(); URL.revokeObjectURL(url);
  }
    ...
    <motion.div initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} className="space-y-4">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          <input value={q} onChange={e => { setQ(e.target.value); setPage(1) }} placeholder="Search user or text" className="border rounded px-3 py-1" />
      ...
      <div className="bg-white rounded-lg shadow-sm overflow-x-auto">
        <table className="min-w-full table-auto">
          <thead className="bg-green-50 text-left text-sm text-green-800">
            <tr><th className="p-4">User</th><th className="p-4">Text</th><th className="p-4">Date</th></tr>
          </thead>
          <tbody>
            {loading ? (<tr><td colSpan={3} className="p-4">Loading...</td></tr>) : current.length === 0 ? (<tr><td colSpan={3} className="p-4">No records.</td></tr>) : current.map(a => (
              <tr key={a._id} className="border-b hover:bg-green-50">
                <td className="p-4">{a.userId?.username || "Unknown"}</td>
                <td className="p-4">{a.text}</td>
                <td className="p-4">{a.dateKey}</td>
      ...
      <div className="flex items-center justify-between text-sm">
        <div>{filtered.length} affirmations</div>
        <div className="flex items-center gap-2">
